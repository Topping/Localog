...
- This repository is the maintained downstream fork `Topping/Localog`.
- `origin` is the only writable GitHub repository and must resolve to `Topping/Localog` before any push or GitHub mutation.
...
- Any explicitly authorized pull request must target `Topping/Localog`, normally with `midnight` as its base branch. Specify the target explicitly, for example with `gh pr create --repo Topping/Localog --base midnight ...`, and verify the resulting base repository before reporting success.
- Upstream synchronization flows from `WoWAnalyzer/WoWAnalyzer` into `Topping/Localog`. It must never open a reverse-direction contribution to the official repository.
...
- Never create a PR unless the user explicitly authorizes a PR to `Topping/Localog` or invokes the project-scoped `upstream_sync` workflow. The fork and GitHub safety rules above always apply.
